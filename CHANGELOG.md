...
- Stock page actions menu: replaced **"Изменить опт. цену"** ("Change WH price") with **"Изменить"** ("Edit"); the action now opens a full edit modal allowing the user to update **barcode**, **purchase price** (`wh_price`), **model**, and **name** for a stock item. +10% and +25% selling prices continue to be derived automatically from the purchase price. Old single-field wholesale-price endpoint is kept for backward compatibility; new `/stock/product/edit` endpoint handles the full edit. i18n updated for EN and RU locales.
- Stock page columns updated: replaced "Опт." and "Прод." with three clearly labeled columns — **"Приход"** (purchase price / `wh_price`), **"+10%"** (`wh10_price`), and **"+25%"** (`wh25_price`); i18n keys renamed accordingly for EN/RU/TK locales.
...
- Update `README.md`: document new auto HTTPS/Basic Auth install flow, non-interactive usage, cert renewal, and adding Basic Auth users
